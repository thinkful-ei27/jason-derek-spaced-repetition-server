...
describe("Spaced Repetition - Users", function() {
  ...
  beforeEach(function() {
    return Promise.all([User.insertMany(users), User.createIndexes()]).then(
      ([users]) => {
        user = users[0];
    ...
    it("should create a new user with lowercase username", function() {
      ...
      return chai
        ...
        .then(user => {
          ...
          expect(user.signs[0]).to.have.keys(
            "sign",
            "answer",
            "m",
            "next",
      ...
      return chai
        ...
        .then(res => {
          ...
          expect(res.body.sign).to.equal(
            `https://${req.host}/signs/${user.signs[0].sign}`
          );
        });
    });
  ...
  describe("POST /api/users/guess", function() {
    it("should return true when given a correct guess regardless of capitalization", function() {
      ...
      return chai
        ...
        .then(res => {
          ...
          expect(res.body.sign.guessesMade).to.equal(
            user.signs[0].guessesMade + 1
          );
          expect(res.body.sign.guessesCorrect).to.equal(
            user.signs[0].guessesCorrect + 1
          );
        });
    });
    ...
    it("should move to the next question after a guess", function() {
      ...
      return chai
        ...
        .then(updatedUser => {
          expect(updatedUser.signs[updatedUser.head].answer).to.not.equal(
            newGuess.guess
          );
        });
    });
    ...
    it("should add the sign to the `learned` list once `m` is 16 or more", function() {
      ...
      return user
        ...
        .then(res => {
          ...
          expect(res.body.user.learned[0].guessesMade).to.equal(
            currSign.guessesMade + 1
          );
          expect(res.body.user.learned[0].guessesCorrect).to.equal(
    ...
    it("should update `guessesMade` and `guessesCorrect` on learned items when guessed correctly", function() {
      ...
      return user
        ...
        .then(user => {
          ...
          expect(user.learned[0].guessesCorrect).to.equal(
